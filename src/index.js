...
function findAllPSiblings(where) {
    ...
    for (let element of where.children) {
        if (element.tagName === 'P') {
            res.push(element.previousElementSibling)
        }
    }
...
function collectDOMStat(root, result = { tags: {}, classes: {}, texts: 0 }) {
    ...
    result.tags[name] += 1;
    if (classname !== '') {
        for (name of classname) {
            if (result.classes[name] === undefined) {
                result.classes[name] = 0;
            }
            result.classes[name] += 1;
        }
    }
    for (let i = 0; i < root.childNodes.length; i++) {
        if (root.childNodes[i].nodeType === 3) {
            ...
            Object.assign(result, collectDOMStat(root.childNodes[i], result))
        }
    }
